...
p2 <- ggplot(data=f.merge,aes(y=log10(all),x=sample,fill=group)) + 
  geom_violin(outlier.shape = NA) + theme_few() + theme(axis.text.x = element_text(angle = 60,hjust=1)) + 
  xlab("") + 
  scale_fill_discrete(name="") + 
  stat_summary(fun.data = "mean_sdl",geom='pointrange',fun.args = (mult=1),color='black',position = position_dodge(0.9)) + 
...
f.table$group <- c(rep('scCT_brain',4),rep('scCT_cell_lines',2),rep('Kaya-Okur',3))
...
f.table$group <- factor(f.table$group,levels=unique(f.table$group))
...
p1
ggsave(filename = paste0(params$out_prefix,'Figures/revision/scCT_QC_FriP.pdf'),plot = p1,width = 8,height = 7)
ggsave(filename = paste0(params$out_prefix,'Figures/revision/scCT_QC_nFragments.pdf'),plot = p2,width = 8,height = 7)
ggsave(filename = paste0(params$out_prefix,'Figures/revision/scCT_QC_nCells.pdf'),plot = p3,width = 8,height = 7)
```
...
p1 <- ggplot(data = pca.plot, aes(x = PC1,y = PC2,label = sample)) + 
  geom_point(aes(color = group),size=5) + 
  geom_text_repel(force=100,size=6,point.padding = 1) +
  theme_few() + theme(legend.position = "None") + coord_cartesian(xlim=c(-1.5,1.5),ylim=c(-0.8,0.5))
...
d.to.plot <- table(brain$cell_type,brain$sample)

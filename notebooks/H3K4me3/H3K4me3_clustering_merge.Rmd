...
saveRDS(brain.merged,"results/H3K4me3/clustering_merged/01.clustering.Rmd")
write.csv2(x = markers,file="results/H3K4me3/clustering_merged/markers.csv")

```

```{r export_bw}
fragments1 <- config$input$H3K4me3$`P15_GFP+`
fragments1 <- rtracklayer::import(fragments1,format = "bed")

fragments2 <- config$input$H3K4me3$`P15_GFP+`
fragments2 <- rtracklayer::import(fragments2,format = "bed")

fragments3 <- config$input$H3K4me3$`P25_GFP+`
...
rtracklayer::export.bw("results/H3K4me3/clustering/bigwig/clusters_all.bw")
...
```{r do_metagene}
clusters_order = levels(brain.marged@active.ident)
...
dir.create("results/H3K4me3/clustering/markers_bed/")
...
lapply(levels(top.markers$cluster),function(x){
  ...
  rtracklayer::export.bed(object = top.markers.x,con = paste0("results/H3K4me3/clustering/markers_cluster_",x,".bed"))
})

bw.files <- list.files(path = "results/H3K4me3/clustering/bigwig/",pattern = "*.bw")

for(f in bw.files){
  system(paste0("/usr/local/anaconda3/bin/computeMatrix reference-point   -S  results/H3K4me3/clustering/bigwig/", f,  
                                                                         " -R results/H3K4me3/clustering/markers_bed/*.bed -o results/H3K4me3/clustering/markers_bed/metagene_",gsub("\\.bw","\\.matrix",f),
                                                                         " -a 10000 ", 
                                                                         " -b 10000 ",
                                                                         " -p 4"))
}
 ...
 for(f in bw.files){
 system(paste0("/usr/local/anaconda3/bin/plotHeatmap   -m results/H3K4me3/clustering/markers_bed/metagene_", gsub("\\.bw","\\.matrix",f), 
                                                     " -o results/H3K4me3/clustering/markers_bed/metagene_", gsub("\\.bw","\\.png",f), 
                                                     " --sortRegions no",
                                                     " --zMin 20",

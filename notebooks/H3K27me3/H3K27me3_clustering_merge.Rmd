...
rtracklayer::export.bw(object = coverage.x, con ="results/H3K27me3/clustering/bigwig/clusters_all.bw")
...
dir.create("results/H3K27me3/clustering/markers_bed/")
...
lapply(levels(top.markers$cluster),function(x){
  ...
  rtracklayer::export.bed(object = top.markers.x,con = paste0("results/H3K27me3/clustering/markers_cluster_",x,".bed"))
})

bw.files <- list.files(path = "results/H3K27me3/clustering/bigwig/",pattern = "*.bw",full.names = TRUE)

for(f in bw.files){
  system(paste0("/usr/local/anaconda3/bin/computeMatrix reference-point   -S  ", f,  
                                                                        " -R results/H3K27me3/clustering/markers_bed/*.bed", 
                                                                         "-o results/H3K27me3/clustering/markers_bed/metagene_",gsub("\\.bw","\\.matrix",f),
                                                                        " -a 10000 ", 
                                                                        " -b 10000 ",
                                                                        " -p 4"))
}
 ...
 for(f in bw.files){
 system(paste0("/usr/local/anaconda3/bin/plotHeatmap   -m results/H3K27me3/clustering/markers_bed/metagene_", gsub("\\.bw","\\.matrix",f), 
                                                     " -o results/H3K27me3/clustering/markers_bed/metagene_", gsub("\\.bw","\\.png",f), 
                                                     " --sortRegions no",
                                                     " --zMin 20",

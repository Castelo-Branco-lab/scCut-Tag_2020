...
```{r load_and_merge}
assay = "peaksMB"
brain.ls <- lapply(names(config$input$H3K27me3),function(x){
  
  if(any(!c("seurat_object","fragments","peaks" ) %in% names(config$input$H3K27me3[[x]]))){
  ...
  brain               <- readRDS(config$input$H3K27me3[[x]]$seurat_object)
  DefaultAssay(brain) <- assay
  
  brain$age <- unlist(strsplit(names(config$input$H3K27me3[x]),"_"))[1]
  brain$GFP <- unlist(strsplit(names(config$input$H3K27me3[x]),"_"))[2]
...
# brain.merged  <- brain.merged[,brain.merged$promoter_ratio < 0.5] # Filter away ATAC-seq like cells
...
p3 <- DimPlot(brain.merged[,brain.merged$sample == 'H3K27me3_P15_GFP+_N1'],label=TRUE)+ NoLegend()
p4 <-  DimPlot(brain.merged[,brain.merged$sample != 'H3K27me3_P15_GFP+_N1'],label=TRUE)+ NoLegend()

p1 + p2
p3 + p4
p2 + coord_cartesian(xlim=c(-15,15),ylim=c(-10,10))


lapply(as.character(colnames(markers.agg.df)),function(x){
  FeaturePlot(brain.merged,x,max.cutoff = quantile(markers.agg.df[,x],0.9,na.rm=TRUE),min.cutoff = quantile(markers.agg.df[,x],0.05,na.rm=TRUE)) + scale_color_viridis_c() + NoLegend()+ coord_cartesian(xlim=c(-15,15),ylim=c(-10,10))
...
small.clusters <- names(small.clusters)[10:length(small.clusters)]
...
saveRDS(brain.merged,"results/H3K27me3/clustering/01.clustering.Rds")
write.csv2(x = markers, file = "results/H3K27me3/clustering/markers.csv")
write.csv2(x = markers.top, file = "results/H3K27me3/clustering/markers_top.csv")

```

```{r export_bw}

fragments1 <- config$input$H3K27me3$`P15_GFP+`$fragments
fragments1 <- rtracklayer::import(fragments1,format = "bed")

fragments2 <- config$input$H3K27me3$`P15_GFP+`$fragments
fragments2 <- rtracklayer::import(fragments2,format = "bed")

fragments3 <- config$input$H3K27me3$`P25_GFP+`$fragments
...
exportBW <- function(object,cluster,fragments){
  ...
  rtracklayer::export.bw(object = coverage.x,paste0("results/H3K27me3/clustering/bigwig/cluster_",cluster,".bw"))
}
...
rtracklayer::export.bw(object = coverage.x, con ="results/H3K27me3/clustering/bigwig/clusters_all/clusters_all.bw")
...
dir.create("results/H3K27me3/clustering/markers_bed/",recursive = TRUE)
...
lapply(levels(top.markers$cluster),function(x){
  ...
  rtracklayer::export.bed(object = top.markers.x,con = paste0("results/H3K27me3/clustering/markers_bed/markers_cluster_",x,".bed"))
})

bw.files <- list.files(path = "results/H3K27me3/clustering/bigwig/",pattern = "*.bw",full.names = TRUE)

system(paste0("/usr/local/anaconda3/bin/computeMatrix reference-point   -S results/H3K27me3/clustering/bigwig/*.bw",  
                                                                      " -R results/H3K27me3/clustering/markers_bed/*.bed", 
                                                                      " -o results/H3K27me3/clustering/markers_bed/metagene_matrix.txt.gz",
                                                                      " -a 10000 ", 
                                                                      " -b 10000 ",
                                                                      " -p 4",sep= ' '))


system(paste0("/usr/local/anaconda3/bin/plotHeatmap   -m results/H3K27me3/clustering/markers_bed/metagene_matrix.txt.gz", 
                                                    " -o results/H3K27me3/clustering/markers_bed/metagene_matrix.pdf", 
                                                    " --sortRegions no",
...
png(filename = "results/H3K27me3/clustering/bins_heatmap.png",width=1000,height=1000)

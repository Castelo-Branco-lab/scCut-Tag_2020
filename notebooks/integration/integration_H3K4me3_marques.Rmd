...
brain.RNA     <- readRDS(file = paste0(params$out_prefix,"marques_RNA/clustering/01.clustering.Rds"))
markers       <- read.csv2(file = paste0(params$out_prefix,"marques_RNA/clustering/markers.csv"),stringsAsFactors = TRUE)
```
...
p.ls <- lapply(colnames(cluster.scores),function(x){
  FeaturePlot(brain.H3K4me3.OL,x,max.cutoff = quantile(brain.H3K4me3.OL@meta.data[,x],0.8)) + scale_color_viridis()
})

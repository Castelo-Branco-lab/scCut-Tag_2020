...
params:
  ...
  out_prefix: "~/mount/CT/git_test/rework/results/" #'results/'
  antibody: 'Rad21'
  window: 25000
---
...
p.ls <- lapply(unique(brain.merged$sample),function(x){
  ...
  DimPlot(to_plot,label=TRUE,pt.size=0.3)+ NoLegend() + ggtitle(paste(x,unique(to_plot$Age), unique(to_plot$GFP))) + theme(plot.title = element_text(size=12)) + coord_cartesian(xlim=c(-15,15),ylim=c(-10,10))
})
...
```{r Rename_the_clusters,fig.width=3,fig.height=3}
# Rename the clusters
brain.merged <- SetIdent(object = brain.merged,
                         cells =  colnames(brain.merged[,brain.merged@active.ident %in% c(3:length(unique(brain.merged@active.ident)))]),
                         value = 'Unknown')
brian.merged <- brain.merged[,brain.merged@active.ident !='Unknown']

brain.merged <- RenameIdents(brain.merged, '0' = 'mOL',
                                    '1' = 'Astrocytes',
                                    '2' = 'OEC')
...
markers.peaks <- FindAllMarkers(brain.merged,min.pct = 0.001,logfc.threshold = 0.1,slot = 'data')

markers.peaks$closest_gene <- ClosestFeature(regions = StringToGRanges(markers.peaks$gene ), annotation = genebodyandpromoter.coords.flat)$name
#View(markers.peaks)

markers.peaks.top <- markers.peaks %>% dplyr::filter(avg_logFC > 0) %>% group_by(cluster) %>% top_n(n = 50,wt = avg_logFC)
markers.peaks.negative <- markers.peaks %>% dplyr::filter(avg_logFC < 0) %>% group_by(cluster) %>% top_n(n = 50,wt = -avg_logFC)

write.csv2(  x = markers.peaks.top, file = paste0(params$out_prefix,params$antibody,"/clustering/peaks_markers_top.csv"))
```
...
# Get top markers
top.markers <- markers.peaks %>% group_by(cluster) %>% dplyr::filter(avg_logFC > 0) %>% top_n(200, wt=avg_logFC)
top.markers <- top.markers %>% dplyr::filter(cluster != "Unknown") %>% droplevels()
